...
function RequestConsent(consent) {
  return getParticipantRegistry('org.data.privacyNetwork.DataUser')
    .then(function (participantRegistry) {
      var factory = getFactory();
      var DataUser = consent.DataUser;
      var consents = DataUser.consents;
      var AssociatedAgent = consent.AssociatedAgent;
      var auths = AssociatedAgent.pushAuthorizations
      for (var i = 0; i < auths.length; i++) {
        var authRoot = auths[i].authRoot;
        if (authRoot.email === DataUser.email){
